...
export async function startServer(config) {
  ...
  app.post('/api/navigate-commit', async (req, res) => {
    ...
    try {
      ...
      let currentBranch = branchInfo.current;
      
      // Check if we're in detached HEAD state
      if (!currentBranch || currentBranch === 'HEAD') {
        return res.status(400).json({ 
          error: 'In detached HEAD state. Please checkout a branch first.' 
        });
      }
      
      // Get the branch's actual tip commit (where the branch ref points)
      // This is important because HEAD might be behind the branch tip
      const branchTipCommit = await git.revparse([`refs/heads/${currentBranch}`]);
      
      // Get all commits from the actual branch ref (not from HEAD)
      // This ensures we see all commits on the branch, including those "ahead" of HEAD
      const logOptions = {
        from: branchTipCommit,
        maxCount: 1000,
        format: {
          hash: '%H',
          abbrevHash: '%h', 
