...
app.post('/api/navigate-commit', async (req, res) => {
  ...
  try {
    ...
    if (direction === 'next') {
      ...
      const { stdout } = await execAsync('git rev-list --children --all | grep "$(git rev-parse HEAD)" | cut -d" " -f2');
...
const server = app.listen(PORT, () => {
  console.log(`Git Review Tool Server running on http://localhost:${PORT}`);
  console.log(`Open http://localhost:${PORT} in your browser to use the review tool`);
});
